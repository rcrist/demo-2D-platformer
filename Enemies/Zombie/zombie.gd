...
@onready var health_progress_bar: ProgressBar = $Control/HealthProgressBar
...
const WALK_SPEED : int = 3000

enum State { Idle, Walk, Attack, Hit }
...
func progress_bar_update():
	health_progress_bar.value = float(health)
...
func _on_attack_area_body_entered(body: Node2D) -> void:
	print("Zombie attack area entered")
...
func _on_attack_area_body_exited(body: Node2D) -> void:
	print("Zombie attack area exited")
	if body.is_in_group("Player"):
		print("Player detected")
		current_state = State.Idle
		timer.start()
