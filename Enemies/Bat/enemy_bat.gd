...
const FLY_SPEED : int = 2500

enum State { Idle, Attack, Die, Fly, Hang, Hit }
...
func enemy_idle(delta: float):
	if !can_fly:
		velocity.x = move_toward(velocity.x, 0, FLY_SPEED * delta)
		current_state = State.Idle
	
func enemy_fly(delta: float):
	if !can_fly:
		return
		
	if abs(position.x -current_point.x) > 0.5:
		velocity.x = direction.x * FLY_SPEED * delta
...
func enemy_animations():
	if current_state == State.Idle and !can_fly:
